								...
								GstSDPMedia *media)
{
	...
	Glib::ustring id, codec_name, clock_rate, channels, extra_attrs, new_extra_attr;
	...
	try {
		...
		for (; it != extra.end(); it ++) {
			try{
				...
				new_extra_attr.append(configFile.get_string(
								codecgrp, *it));
			} catch(Glib::KeyFileError ex) {
				new_extra_attr = "";
			}
	...
	rtpmap = "";
	rtpmap.append(id).append(" ").append(codec_name).append("/").append(clock_rate);
	if (channels != "")
...
static void
load_codec(Glib::KeyFile &configFile, const std::string &codecgrp, GstSDPMedia *media)
{
	GST_DEBUG("Loading config for: %s", codecgrp.c_str());
...
load_session_descriptor(Glib::KeyFile &configFile)
{
	...
	gst_sdp_message_set_origin(sdp_message, "-", DEFAULT_ID, "0", "IN", "IP4", "localhost");
