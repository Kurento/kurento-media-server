...
RegionOfInterest::RegionOfInterest (const Json::Value &value) {
  ...
  if (value.isMember ("regionOfInterestConfig")) {
    aux = value["regionOfInterestConfig"];

    if (!aux.isString ()) {
      /* param 'regionOfInterestConfig' has invalid type value, raise exception */
      JsonRpc::CallException e (JsonRpc::ErrorCode::SERVER_ERROR_INIT,
                                "'regionOfInterestConfig' parameter should be a string");
      throw e;
    } else {
      JsonSerializer s(false);
      s.JsonValue = value;
      s.SerializeNVP(regionOfInterestConfig);
    }
  } else {
    /* Requiered property 'regionOfInterestConfig' not present, raise exception */
    JsonRpc::CallException e (JsonRpc::ErrorCode::SERVER_ERROR_INIT,
                              "'regionOfInterestConfig' property is requiered");
    throw e;
  }
...
Serialize(std::shared_ptr<kurento::RegionOfInterest>& object, JsonSerializer& s)
{
  ...
  if (object) {
    ...
    s.Serialize("regionOfInterestConfig", object->regionOfInterestConfig);
