...
BOOST_AUTO_TEST_CASE ( rtp_end_point_test )
{
  ...
  BOOST_REQUIRE_NO_THROW (client->createMediaElement (sdpEpA, mediaPipeline,
                          g_KmsMediaRtpEndPointType_constants.TYPE_NAME) );
  BOOST_REQUIRE_NO_THROW (client->createMediaElement (sdpEpB, mediaPipeline,
                          g_KmsMediaRtpEndPointType_constants.TYPE_NAME) );
  ...
  try {
    ...
    BOOST_CHECK_EQUAL (
      g_KmsMediaErrorCodes_constants.SDP_END_POINT_NO_LOCAL_SDP_ERROR, e.errorCode);
  }

  try {
    client->invoke (ret, sdpEpA, g_KmsMediaSdpEndPointType_constants.GET_REMOTE_SDP,
                    emptyParams);
    BOOST_FAIL ("Get local SDP without negotiation must throw KmsMediaServerException");
  } catch (const KmsMediaServerException &e) {
    BOOST_CHECK_EQUAL (
      g_KmsMediaErrorCodes_constants.SDP_END_POINT_NO_REMOTE_SDP_ERROR, e.errorCode);
  }

  BOOST_REQUIRE_NO_THROW (client->invoke (ret, sdpEpA,
                                          g_KmsMediaSdpEndPointType_constants.GENERATE_SDP_OFFER, emptyParams) );
  BOOST_REQUIRE_NO_THROW (unmarshalStringInvocationReturn (sdp, ret) );

  std::map<std::string, KmsMediaParam> paramsProcessOffer;
  setStringParam (paramsProcessOffer,
                  g_KmsMediaSdpEndPointType_constants.PROCESS_SDP_OFFER_PARAM_OFFER_STR, sdp);
  BOOST_REQUIRE_NO_THROW (client->invoke (ret, sdpEpB,
                                          g_KmsMediaSdpEndPointType_constants.PROCESS_SDP_OFFER, paramsProcessOffer) );
  BOOST_REQUIRE_NO_THROW (unmarshalStringInvocationReturn (sdp, ret) );

  std::map<std::string, KmsMediaParam> paramsProcessAnswer;
  setStringParam (paramsProcessAnswer,
                  g_KmsMediaSdpEndPointType_constants.PROCESS_SDP_ANSWER_PARAM_ANSWER_STR, sdp);
  BOOST_REQUIRE_NO_THROW (client->invoke (ret, sdpEpA,
                                          g_KmsMediaSdpEndPointType_constants.PROCESS_SDP_ANSWER, paramsProcessAnswer) );
  BOOST_REQUIRE_NO_THROW (unmarshalStringInvocationReturn (sdp, ret) );

  BOOST_REQUIRE_NO_THROW (client->invoke (ret, sdpEpA,
                                          g_KmsMediaSdpEndPointType_constants.GET_LOCAL_SDP, emptyParams) );
  BOOST_REQUIRE_NO_THROW (unmarshalStringInvocationReturn (localSdpA, ret) );

  BOOST_REQUIRE_NO_THROW (client->invoke (ret, sdpEpA,
                                          g_KmsMediaSdpEndPointType_constants.GET_REMOTE_SDP, emptyParams) );
